...
def _bootstrap():
    """Initialise DB once per Streamlit session. Returns settings object."""
...
st.markdown(
    ...
    .badge-ok  {{ background:#1a4731; color:{C['green']}; border:1px solid {C['green']};
                  border-radius:8px; padding:.3rem .85rem; font-weight:700;
        ...
        fig_d.update_layout(
            **_PLOTLY, height=320, showlegend=True,
            ...
            annotations=[dict(
                ...
                x=0.5, y=0.5, font=dict(size=13, color="#fff"), showarrow=False,
            )],
        )
...
if not ts_df.empty:
    ...
    st.info(
        "No data yet.  \n"
        "**Run locally:** `python recover_ai/data_simulator.py --burst 30`",
        icon="📡",
    )
...
if not txn_df.empty and "recoverability_score" in txn_df.columns:
    ...
    if not scores.empty:
        ...
        fig_h.add_vline(
            x=_settings.ml_low_priority_threshold,
            line_dash="dash", line_color=C["red"], line_width=2,
            annotation_text=f"Skip Threshold ({_settings.ml_low_priority_threshold})",
        ...
        st.info("ML scoring not yet triggered — send events first.", icon="⚙️")
else:
...
if st.button("🔍 Verify Ledger Integrity Now", type="primary"):
    ...
    if ok:
        ...
        st.markdown(
            ...
            '⚠️ TAMPER DETECTED</div>',
            unsafe_allow_html=True,
        )
        st.error(msg)

# ── Audit Trail ───────────────────────────────────────────────────────────────
with st.expander("📜 Audit Trail — Immutable SHA-256 Hash-Chain Log", expanded=False):
    ...
    if not audit_df.empty:
        ...
        st.dataframe(adf, use_container_width=True, height=340)
        st.caption(f"{len(audit_df)} records")

        with st.expander("🔎 Raw JSON Inspector"):
            idx = st.number_input(
                "Row index", min_value=0,
